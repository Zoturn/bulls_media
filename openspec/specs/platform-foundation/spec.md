...
## Purpose
The durable substrate every later capability builds on: the data model for an enquiry and its
processing (message, case, run, step, assessment, approval, and the rate-card and policy reference
tables), a deterministic seed covering the ordinary and adversarial paths, validated configuration,
redacted structured logging, the shared HTTP error envelope, and a health check. No agent, tool or
model call exists yet — this capability is what makes every later one testable and inspectable.
