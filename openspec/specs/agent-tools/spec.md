...
## Purpose
The five deterministic, offline functions the orchestrator calls: checking a vertical against ad
policy, searching the rate card, confirming inventory for a specific package, computing a quote's
arithmetic, and saving the final assessment — the only write in the system. None of them touch a
model; each is fully specified and independently tested so a later change can build the run loop
around a fixed, trusted surface rather than co-designing tool behaviour under time pressure.
