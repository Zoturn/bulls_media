...
Making the agent's claims checkable, so that what gets recorded is what the tools support.

The orchestration capability decides what the agent can reach. This one decides what the system
will believe. They fail differently: a phase gate holds against a model trying to get past it,
while these checks hold when every tool call was legitimate and the model simply wrote the wrong
number down — the more likely failure of the two.

Two things follow from that. Every value the agent asserts is compared against the recorded tool
results before it is persisted, by one pure function run at each boundary where a claim becomes a
record — so a model that disagrees with itself is caught at whichever it reaches second. And a
refusal is a fact the policy engine established about the run, not a conclusion the model reached:
it takes effect whether or not the model agrees, and whether or not the model answered at all.

It covers the post-conditions and where they run, the enforced refusal and the evidence recorded
with it, the treatment of every string reaching the model from outside the system prompt —
inbound mail and retrieved documents alike — and the injection corpus the whole lot is asserted
against, by behaviour rather than by wording.

What it deliberately does not do is detect instructions semantically. There is no classifier
judging whether a sentence is an injection, because that would be another non-deterministic step
needing a guarantee of its own. The defence here is structural: untrusted text cannot reach the
system prompt, tools cannot be reached out of phase, and numbers cannot be recorded without a
